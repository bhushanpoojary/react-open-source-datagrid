...
export class GridApiImpl {
    ...
    applyTransaction(transaction) {
        ...
        if (transaction.add && transaction.add.length > 0) {
            ...
            result.add = transaction.add.map((r, idx) => this.createRowNode(r, addIndex + idx));
        }
    ...
    getDisplayedCenterColumns() {
        ...
        return this.state.columnOrder
            .filter((field) => !pinnedLeft.has(field) && !pinnedRight.has(field) && !hidden.has(field))
    ...
    ensureIndexVisible(index, position) {
        ...
        if (!this.containerRef.current)
            return;
        const rowElement = this.containerRef.current.querySelector(`[data-row-index="${index}"]`);
        if (rowElement) {
            const scrollOptions = {
                behavior: 'smooth',
    ...
    sizeColumnsToFit() {
        ...
        if (visibleColumns.length === 0)
            return;
        const totalCurrentWidth = visibleColumns.reduce((sum, col) => sum + (this.state.columnWidths[col.field] || col.width || 150), 0);
