...
export const GridBody = ({ columns, rows, pinnedRowsTop = [], pinnedRowsBottom = [], columnOrder, displayColumnOrder, columnWidths, selectedRows, editState, focusState, dispatch, onRowClick, onCellEdit, pinnedLeft, pinnedRight, showGroupFooters = false, groupAggregates = new Map(), aggregateConfigs = [], virtualScrollConfig, treeConfig, dragRowConfig, tableId, onRowReorder, onScroll, currentPage = 0, pageSize = 10, totalRows = 0, onContextMenu, onCellMouseEnter, onCellMouseLeave, onRowMouseEnter, onRowMouseLeave, }) => {
    ...
    const renderRowContent = (row, rowIndex, style) => {
        ...
        if (isGroupedRow(row)) {
            ...
            return (_jsxs(React.Fragment, { children: [_jsx(GroupRow, { group: row, columns: columns, columnOrder: columnOrder, displayColumnOrder: displayColumnOrder, columnWidths: columnWidths, dispatch: dispatch, pinnedLeft: pinnedLeft, pinnedRight: pinnedRight }), showGroupFooters && row.isExpanded && aggregateConfigs.length > 0 && (_jsx(GroupFooterRow, { group: row, columns: columns, displayColumnOrder: displayColumnOrder, columnWidths: columnWidths, aggregates: groupAgg, aggregateConfigs: aggregateConfigs, pinnedLeft: pinnedLeft, pinnedRight: pinnedRight }))] }, row.groupKey));
        }
        // Check if this is a tree node
        if (treeConfig?.enabled && isTreeNode(row)) {
            return (_jsx(TreeRow, { node: row, columns: columns, columnOrder: columnOrder, displayColumnOrder: displayColumnOrder, columnWidths: columnWidths, selectedRows: selectedRows, editState: editState, focusState: focusState, rowIndex: rowIndex, dispatch: dispatch, onRowClick: onRowClick, onCellEdit: onCellEdit, pinnedLeft: pinnedLeft, pinnedRight: pinnedRight, treeConfig: treeConfig, editInputRef: editInputRef }, row.id));
        }
        ...
        const rowContent = (_jsxs("div", { role: "row", "aria-rowindex": rowIndex + 2, "aria-selected": isSelected, "data-row-index": rowIndex, style: {
                ...style,
                display: 'flex',
                minWidth: '100%',
                ...
                transition: 'background-color 0.15s ease',
            }, onMouseEnter: (e) => !isSelected && !isLoadingRow && (e.currentTarget.style.backgroundColor = 'var(--grid-hover)'), onMouseLeave: (e) => !isSelected && !isLoadingRow && (e.currentTarget.style.backgroundColor = 'var(--grid-bg)'), onClick: (e) => !isLoadingRow && handleRowClick(row, rowIndex, e), children: [showDragHandle && handlePosition === 'left' && (_jsx("div", { style: {
                        width: '32px',
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    }, children: _jsxs("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "currentColor", children: [_jsx("circle", { cx: "7", cy: "5", r: "1.5" }), _jsx("circle", { cx: "13", cy: "5", r: "1.5" }), _jsx("circle", { cx: "7", cy: "10", r: "1.5" }), _jsx("circle", { cx: "13", cy: "10", r: "1.5" }), _jsx("circle", { cx: "7", cy: "15", r: "1.5" }), _jsx("circle", { cx: "13", cy: "15", r: "1.5" })] }) })), displayColumnOrder.map((field, columnIndex) => {
                    const column = columnMap.get(field);
                    if (!column)
                        return null;
                    const cellValue = isLoadingRow ? '' : row[field];
                    const isEditing = editState.rowId === row.id && editState.field === field;
                    const isCellFocused = focusState?.rowIndex === rowIndex &&
                        focusState?.columnIndex === columnIndex;
                    const cellStyle = getPinnedCellStyle(field);
                    return (_jsx("div", { ref: isCellFocused ? focusedCellRef : null, role: "gridcell", "aria-colindex": columnIndex + 1, "aria-readonly": column.editable === false, style: {
                            ...cellStyle,
                            ...
                            outlineOffset: '-2px',
                            color: 'var(--grid-text)',
                        }, onDoubleClick: () => handleCellDoubleClick(row, field, cellValue), onContextMenu: (e) => onContextMenu?.(e, row, column, rowIndex, columnIndex), onMouseEnter: (e) => onCellMouseEnter?.(e, row, column, cellValue), onMouseLeave: () => onCellMouseLeave?.(), onKeyDown: (e) => handleKeyDown(e, rowIndex, columnIndex), tabIndex: isCellFocused ? 0 : -1, children: isEditing ? (_jsx("input", { ref: editInputRef, type: "text", style: {
                                width: '100%',
                                padding: '6px 8px',
                                ...
                                color: 'var(--grid-text)',
                            }, value: editState.value ?? '', onChange: (e) => handleEditChange(e.target.value), onBlur: handleEditComplete, onKeyDown: (e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                                    handleEditComplete();
                                }
                            ...
                            }, children: isLoadingRow ? (_jsx("span", { style: {
                                ...
                                } })) : (column.renderCell ? column.renderCell(row) : (cellValue ?? '')) })) }, `${row.id}-${field}`));
                }), showDragHandle && handlePosition === 'right' && (_jsx("div", { style: {
                        width: '32px',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        ...
                        e.currentTarget.style.color = 'var(--grid-text-secondary, #9ca3af)';
                        e.currentTarget.style.backgroundColor = 'transparent';
                    }, children: _jsxs("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "currentColor", children: [_jsx("circle", { cx: "7", cy: "5", r: "1.5" }), _jsx("circle", { cx: "13", cy: "5", r: "1.5" }), _jsx("circle", { cx: "7", cy: "10", r: "1.5" }), _jsx("circle", { cx: "13", cy: "10", r: "1.5" }), _jsx("circle", { cx: "7", cy: "15", r: "1.5" }), _jsx("circle", { cx: "13", cy: "15", r: "1.5" })] }) }))] }, row.id));
        // Wrap in DraggableRow if drag is enabled
        if (dragRowConfig?.enabled && !isLoadingRow) {
            return (_jsx(DraggableRow, { row: row, rowIndex: rowIndex, config: dragRowConfig, sourceTableId: tableId, onRowReorder: onRowReorder, rows: rows, style: style, children: rowContent }, row.id));
        }
        return rowContent;
    };
    // Render pinned rows (top)
    const renderPinnedTopRows = () => {
        if (pinnedRowsTop.length === 0)
            return null;
        return (_jsx("div", { style: { position: 'sticky', top: 0, zIndex: 15, backgroundColor: 'var(--grid-bg)' }, children: pinnedRowsTop.map((row, rowIndex) => renderRowContent(row, rowIndex)) }));
    };
    // Render pinned rows (bottom)
    const renderPinnedBottomRows = () => {
        if (pinnedRowsBottom.length === 0)
            return null;
        return (_jsx("div", { style: { position: 'sticky', bottom: 0, zIndex: 15, backgroundColor: 'var(--grid-bg)' }, children: pinnedRowsBottom.map((row, rowIndex) => renderRowContent(row, rowIndex + rows.length + pinnedRowsTop.length)) }));
    };
    ...
    if (virtualScrollConfig?.enabled) {
        ...
        return (_jsxs("div", { ref: bodyRef, role: "rowgroup", style: { position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', width: '100%', backgroundColor: 'var(--grid-bg)' }, children: [renderPinnedTopRows(), _jsx(VirtualScroller, { items: rows, itemHeight: virtualScrollConfig.rowHeight || 35, overscanCount: virtualScrollConfig.overscanCount || 5, containerHeight: virtualScrollConfig.containerHeight || 600, columns: enableColumnVirtualization ? virtualColumns : [], totalColumnWidth: enableColumnVirtualization ? totalColumnWidth : 0, columnOverscan: virtualScrollConfig.columnOverscan || 3, onScroll: onScroll, renderItem: (row, index, style) => renderRowContent(row, index + pinnedRowsTop.length, style), renderRow: enableColumnVirtualization ? (row, index, visibleColumns, style) => {
                        if (isGroupedRow(row)) {
                        ...
                        return (_jsx("div", { role: "row", "aria-rowindex": index + 2, "aria-selected": isSelected, style: {
                                ...style,
                                display: 'flex',
                                minWidth: '100%',
                                borderBottom: 'var(--grid-border-width, 1px) solid var(--grid-border)',
                                ...
                                if (!isSelected)
                                    e.currentTarget.style.backgroundColor = 'var(--grid-bg)';
                                onRowMouseLeave?.();
                            }, onClick: (e) => handleRowClick(row, index, e), children: visibleColumns.map((colInfo, columnIndex) => {
                                ...
                                const isEditing = editState.rowId === row.id && editState.field === field;
                                const isCellFocused = focusState?.rowIndex === index &&
                                    focusState?.columnIndex === displayColumnOrder.indexOf(field);
                                return (_jsx("div", { ref: isCellFocused ? focusedCellRef : null, role: "gridcell", "aria-colindex": displayColumnOrder.indexOf(field) + 1, "aria-readonly": column.editable === false, style: {
                                        ...
                                        outline: isCellFocused ? '2px solid var(--grid-primary)' : 'none',
                                        color: 'var(--grid-text)',
                                    }, onDoubleClick: () => handleCellDoubleClick(row, field, cellValue), onContextMenu: (e) => onContextMenu?.(e, row, column, index, columnIndex), onMouseEnter: (e) => onCellMouseEnter?.(e, row, column, cellValue), onMouseLeave: () => onCellMouseLeave?.(), onKeyDown: (e) => handleKeyDown(e, index, columnIndex), tabIndex: isCellFocused ? 0 : -1, children: isEditing ? (_jsx("input", { ref: editInputRef, type: "text", style: { width: '100%', padding: '4px', border: '1px solid var(--grid-primary)', borderRadius: 'var(--grid-border-radius, 4px)', outline: 'none', backgroundColor: 'var(--grid-bg)', color: 'var(--grid-text)' }, value: editState.value ?? '', onChange: (e) => handleEditChange(e.target.value), onBlur: handleEditComplete, onKeyDown: (e) => {
                                            if (e.key === 'Enter') {
                                                e.preventDefault();
                                                handleEditComplete();
                                            }
                                        ...
                                        } })) : (_jsx("span", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }, children: cellValue ?? '' })) }, `${row.id}-${field}`));
                            }) }));
                    } : undefined }), renderPinnedBottomRows()] }));
    }
    // Non-virtual scrolling mode (original implementation)
    return (_jsxs("div", { ref: bodyRef, role: "rowgroup", style: { overflow: 'auto', maxHeight: '500px', position: 'relative', backgroundColor: 'var(--grid-bg)', width: '100%' }, children: [renderPinnedTopRows(), rows.map((row, rowIndex) => renderRowContent(row, rowIndex + pinnedRowsTop.length)), renderPinnedBottomRows()] }));
};
