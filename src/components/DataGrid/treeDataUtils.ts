 ...
 */
export const isTreeNode = (row: Row | TreeNode): row is TreeNode => {
...
export const buildTreeFromFlat = (
  ...
  nodeMap.forEach(node => {
    ...
    if (parentId === null || parentId === undefined || !nodeMap.has(parentId)) {
      ...
      const childrenArray = parent[childrenField] as TreeNode[];
  ...
  const setLevels = (nodes: TreeNode[], level: number) => {
    nodes.forEach(node => {
      node.level = level;
      const children = node[childrenField] as TreeNode[];
...
export const getDescendantIds = (
  ...
  const traverse = (currentNode: TreeNode) => {
    const children = currentNode[childrenField] as TreeNode[];
    if (children && children.length > 0) {
...
export const getNodePath = (
  ...
  const search = (nodeArray: TreeNode[], path: (string | number)[]): (string | number)[] | null => {
...
export const getTreeDepth = (
  ...
  const traverse = (nodeArray: TreeNode[], depth: number) => {
    nodeArray.forEach(node => {
...
export const filterTree = (
  ...
  const filter = (nodeArray: TreeNode[]): TreeNode[] => {
    const result: TreeNode[] = [];
