      ...
      Object.keys(sampleRow).forEach(key => {
        if (key !== 'id' && key !== field) {
          ...
          if (numericValues.length > 0) {
            aggregates[`${key}_sum`] = numericValues.reduce((a: number, b: number) => a + b, 0);
 ...
 */
export const flattenGroupedRows = (
  groupedRows: (Row | GroupedRow)[]
): (Row | GroupedRow)[] => {
  const result: (Row | GroupedRow)[] = [];
 ...
 */
export const isGroupedRow = (row: Row | GroupedRow): row is GroupedRow => {
