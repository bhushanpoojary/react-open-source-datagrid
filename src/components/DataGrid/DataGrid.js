...
export const DataGrid = forwardRef(({ columns, rows, pageSize = 10, showColumnPinning = true, footerConfig, virtualScrollConfig, persistenceConfig, treeConfig, dragRowConfig, rowPinConfig, contextMenuConfig, tooltipConfig, tableId, theme: _theme = 'quartz', densityMode: _densityMode = 'normal', showDensityToggle = false, onDensityChange, onRowClick, onCellEdit, onSelectionChange, onLayoutChange, onRowReorder, onGridReady, }, ref) => {
    ...
    return (_jsxs("div", { ref: containerRef, "data-testid": "data-grid", role: "grid", "aria-label": "Data Grid", "aria-rowcount": flattenedRows.length, "aria-colcount": displayColumnOrder.length, style: {
            ...themeStyles,
            ...densityStyles,
            ...
            fontFamily: 'var(--grid-font-family, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif)'
                            ...
                            columnIndex,
                            event,
                        }) }), _jsx(ColumnFilters, { columns: columns, displayColumnOrder: displayColumnOrder, columnWidths: state.columnWidths, filterConfig: state.filterConfig, dispatch: dispatch, pinnedLeft: pinnedLeftFields, pinnedRight: pinnedRightFields, rows: filteredRows })] }), _jsx(GridBody, { columns: columns, rows: virtualScrollConfig?.enabled ? unpinnedRows : paginatedRows, pinnedRowsTop: pinnedRowsTopData, pinnedRowsBottom: pinnedRowsBottomData, columnOrder: state.columnOrder, displayColumnOrder: displayColumnOrder, columnWidths: state.columnWidths, selectedRows: state.selection.selectedRows, editState: state.editState, focusState: state.focusState, dispatch: dispatch, onRowClick: onRowClick, onCellEdit: handleCellEdit, pinnedLeft: pinnedLeftFields, pinnedRight: pinnedRightFields, showGroupFooters: footerConfig?.showGroupFooters, groupAggregates: groupAggregates, aggregateConfigs: footerConfig?.aggregates, virtualScrollConfig: virtualScrollConfig, treeConfig: treeConfig, dragRowConfig: dragRowConfig, tableId: tableId, onRowReorder: onRowReorder, currentPage: state.currentPage, pageSize: state.pageSize, totalRows: unpinnedRows.length, onContextMenu: (event, row, column, rowIndex, columnIndex) => handleContextMenu({
                    type: 'cell',
                    row,
                    column,
                    rowIndex,
                    columnIndex,
                    event,
                }), ...tooltipHandlers }), footerConfig?.show && footerConfig.aggregates && (_jsx(GridFooter, { columns: columns, displayColumnOrder: displayColumnOrder, columnWidths: state.columnWidths, aggregates: globalAggregates, aggregateConfigs: footerConfig.aggregates, pinnedLeft: pinnedLeftFields, pinnedRight: pinnedRightFields })), !virtualScrollConfig?.enabled && (_jsx(GridPagination, { currentPage: state.currentPage, pageSize: state.pageSize, totalRows: unpinnedRows.length, dispatch: dispatch })), contextMenu.isOpen && (_jsx(ContextMenu, { x: contextMenu.x, y: contextMenu.y, items: contextMenu.items, onClose: closeContextMenu })), tooltipConfig?.enabled && (_jsx(Tooltip, { state: tooltipState, maxWidth: tooltipConfig.maxWidth, offset: tooltipConfig.offset }))] }));
});
