 ...
 */
export const copyToClipboard = async (selectedRows, columns, rows, includeHeaders = false) => {
 ...
 */
export const copyCellRange = async (startRow, endRow, startCol, endCol, columns, rows, includeHeaders = false) => {
 ...
 */
export const exportSelectedToCSV = (selectedRows, columns, rows, filename = 'export.csv') => {
    const selectedRowIds = Array.from(selectedRows);
    const selectedData = rows.filter(row => selectedRowIds.includes(row.id));
    if (selectedData.length === 0) {
        return;
 ...
 */
export const calculateOptimalWidth = (column, rows, minWidth = 60, maxWidth = 500, padding = 32) => {
 ...
 */
export const autoSizeAllColumns = (columns, rows) => {
    const widths = {};
 ...
 */
export const createFilterByValue = (value) => {
    return {
        type: 'equals',
        value: value,
    };
 ...
 */
export const getUniqueColumnValues = (column, rows, maxValues = 100) => {
