...
interface TreeRowProps {
  ...
  editState: { rowId: string | number | null; field: string | null; value: any };
  focusState: { rowIndex: number; columnIndex: number } | null;
  rowIndex: number;
...
export const TreeRow: React.FC<TreeRowProps> = ({
  ...
  const renderCell = (field: string, columnIndex: number) => {
    const column = columnMap.get(field);
    if (!column) return null;

    const isFirstColumn = columnIndex === 0;
    const isFocused =
      focusState?.rowIndex === rowIndex && focusState?.columnIndex === columnIndex;
          ...
          >
            {node.hasChildren ? (
              ...
              <div style={{ 
                ...
                {/* Visual connector for leaf nodes */}
                <div style={{
                  position: 'absolute',
                  left: '9px',
                  top: '0',
