 ...
 */
export const Tooltip = ({ state, maxWidth = 300, offset = 8, }) => {
    const tooltipRef = useRef(null);
    const [position, setPosition] = useState({ x: 0, y: 0 });
    const [actualPlacement, setActualPlacement] = useState(state.placement);
    ...
    const content = typeof state.content === 'string'
        ...
        : state.content;
    const tooltipElement = (_jsxs("div", { ref: tooltipRef, role: "tooltip", "aria-live": "polite", style: {
            position: 'fixed',
            left: `${position.x}px`,
            top: `${position.y}px`,
            ...
            transition: 'opacity 0.15s ease-in-out',
                    ...
                    wordWrap: 'break-word',
                }, children: [content.title && (_jsx("div", { style: {
                            fontWeight: '600',
                            marginBottom: '4px',
                            fontSize: '14px',
                        }, children: content.title })), _jsx("div", { children: content.content })] }), _jsx("div", { style: {
                    position: 'absolute',
                    width: 0,
                    height: 0,
                    borderStyle: 'solid',
